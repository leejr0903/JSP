...
<%
Date today = new Date(); // 서버의 현재시간
//Date today = new Date(111,5,7,9,0,0); // 현재시간 조절하여 테스트
// 년도의 기준이 1900 이므로 111은 1900+111=2011년도를 나타낸다.
Date beginDate = new Date(111,5,7,9,0,0); // 시작시간
Date endDate = new Date(111,5,20,18,0,0); // 마감시간
%>

<h4>JSP Time Period Test</h4>
현재시간은 <%= today %> 입니다.<br>
시작시간은 <%= beginDate %> 입니다.<br>
마감시간은 <%= endDate %> 입니다.<br>
-------------------------------------<br>
현재시간을 밀리세컨단위로 변환하면 <%= today.getTime() %> 입니다.<br>
시작시간을 밀리세컨단위로 변환하면 <%= beginDate.getTime() %> 입니다.<br>
마감시간을 밀리세컨단위로 변환하면 <%= endDate.getTime() %> 입니다.<br>
